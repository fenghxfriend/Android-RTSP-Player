		...
		if(CNetBase::UnpackPacket(m_RecvUnpacker.m_aBuffer, Bytes, &m_RecvUnpacker.m_Data) == 0)
		{
			...
			else
			{
				...
				if(m_RecvUnpacker.m_Data.m_Flags&NET_PACKETFLAG_CONTROL)
				{
				    ...
				    {
                        //prevent connectting that not begin from NET_CTRLMSG_CONNECT
                        if(Found)
                        {
                            //predict the online state
						    if(m_aSlots[slotNo].m_Connection.State() == NET_CONNSTATE_PENDING && m_RecvUnpacker.m_Data.m_aChunkData[0] == NET_CTRLMSG_ACCEPT)
                            {
                                //change state to online
                                if(m_aSlots[slotNo].m_Connection.Feed(&m_RecvUnpacker.m_Data, &Addr))
                                {
                                    if(m_pfnNewClient)
                                        m_pfnNewClient(slotNo, m_UserPtr);
                                }
                            }
                            else
                            {
                                if (g_Config.m_Debug > 2) dbg_msg("connect", "get normal ctrl msg\n");
                                m_aSlots[slotNo].m_Connection.Feed(&m_RecvUnpacker.m_Data, &Addr);
                            }
				...
				else
				{
					...
					for(int i = 0; i < MaxClients(); i++)
					{
						...
						if(net_addr_comp(&PeerAddr, &Addr) == 0)
						{
							...
							{
                                if (g_Config.m_Debug > 2) dbg_msg("connect", "get client %d reqest normal msg!\n", i);
