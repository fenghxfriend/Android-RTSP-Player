...
#define GAudioRecorderQueueDefaultCapacity  ( 50 * 1024 ) // 50KB

GAudioRecorderQueue::GAudioRecorderQueue (			  	) { init   ( GAudioRecorderQueueDefaultCapacity	); }
GAudioRecorderQueue::GAudioRecorderQueue ( int capacity ) { init   ( capacity 					   		); }
GAudioRecorderQueue::~GAudioRecorderQueue(			  	) { destory(								  	); }

void GAudioRecorderQueue::init(int capacity)
{
	//检查参数
	if ( capacity <= 0 ) capacity = GAudioRecorderQueueDefaultCapacity;
...
void GAudioRecorderQueue::destory()
{
	mUtility.lock(mLock);
	free( mBufferPtr );
	mDataBytes 			= 0;
...
void GAudioRecorderQueue::clear()
{
	mUtility.lock(mLock);
	mDataBytes 		= 0;
	mDataPtrOffset	= 0;
...
void GAudioRecorderQueue::write(char *data, int size)
{
	do
	{
		//判断参数是否有效
...
int GAudioRecorderQueue::read(char *buffer, int maxSize)
{
	int result = 0;
...
void GAudioRecorderQueue::write0(char *data, int size, bool lock)
{
	//加锁
...
void GAudioRecorderQueue::expandCapacity()
{
	//重新申请buffer
